...
const RoutineBox = ({
  ...
  return (
    <Wrap>
      {modalLoginRequest?<ModalRequestLogin offLoginModal={offLoginModal}/>:''}
      {modalRoutineTitle?<ModalRoutineTitle offModalRoutinetitle={offModalRoutinetitle} saveRoutineTitle={saveRoutineTitle}/>:''}
      ...
      <ContentWrap>
        ...
        <SummaryHead>Summary</SummaryHead>
          <EachSummary>
            <Tag>예상 소요 시간</Tag>
            <Content>{`${(addedWorkout.reduce((acc, cur) => acc+(cur.image.length === 2?cur.myCount:0), 0))/60} Min`}</Content>
          </EachSummary>
          <EachSummary>
            <Tag>총 소요 칼로리</Tag>
            <Content>{`${numberWithCommas(addedWorkout.reduce((acc, cur) => acc + (cur.calrorie*(Math.round(cur.myCount/cur.count)*Math.round(cur.mySetCount/cur.setCount))), 0))} kcal`}</Content>
          </EachSummary>
          <EachSummary>
            <Tag>총 운동 갯수</Tag>
        ...
        <ControlBtn>
          <SaveBtn type="button" value="Save Routine" onClick={clickSaveRoutine}/>
          <RunBtn type="button" value="Run Routine" onClick={() => setCurretRoutine(cards)}/>
        </ControlBtn>
      </ContentWrap>
    </Wrap>
  );
