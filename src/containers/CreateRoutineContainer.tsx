...
import { ModalConfirm, ModalCustomizeWorkout, ModalWorkoutDetail } from '../components/modal'
import { actionSetMyRoutines, actionSetCurrentRoutine, actionRenewToken, actionSetMyWorkouts, actionExpired } from '../modules/actions'
import { URI } from '..';
...
const CreateRoutineContainer = ():JSX.Element => {
    ...
    const removeWorkout = async () => {
        ...
        if (isTokenValid) {
            axios.post(`${URI}/myroutine/removeworkout`, {workoutId:selectedId}, {
                headers:{
    ...
    const editMyWorkout = ({mySetCount, myCount, myBreakTime}:CustomWorkout):void => {
        ...
        tempArr[targetIdx] = {...targetEl, ...{mySetCount, myCount, myBreakTime}}
    ...
    const saveMyRoutine = async (title:string) => {
        ...
        let isTokenValid = await actionRenewToken(token, expDate, dispatch)
        if (isTokenValid) {
            axios.post(`${URI}/myroutine/createroutine`, {workouts:addedWorkout, title}, {
                headers:{
                    'Content-Type':'application/json',
                    'Authorization':`Bearer ${auth.token}`
                }
